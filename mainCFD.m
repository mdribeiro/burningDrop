...
Mu  =     4*1.0E-5;  %for Re = 600  % 2*1.0E-5 for Re = 1200 and 1.0E-5 for Re = 2400   % [ Pa.s ] Dynamic viscosity  
...
Nt  = 15000;  %80000      ;% [  --  ] Number of time-steps
...
for n=1:Nt
    ...
    %% Evaporation Model by Miller and Sazhin
...
%       RhoU(1:Imap2,Jmidm-floor(JJet/2):Jmidp+ceil(JJet/2)) = Rho*Mag;
    ...
    % Adjust velocities to consider pressure forcess
    ...
    if (mod(n,100) == 0); pause(1.0); 
        ...
        title({['Time = ',num2str(T),' s'];
            %['Time-step = ',num2str(cont1)]s
            %['Re = ',num2str(Re)]
            ['r = ',num2str(r0),' mm']
            ['m = ',num2str(m0*1000),' g']
            },'FontWeight','bold','FontSize',12);
        
        imgname = strcat('Burn_Re=',num2str(Re));
        imgname = strcat(imgname,'_N=');
        imgname = strcat(imgname,num2str(cont1));
